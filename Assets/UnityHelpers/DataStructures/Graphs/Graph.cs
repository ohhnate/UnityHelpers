...
public class Graph<T>
{
    ...
    public int NumNodes => _adjacencyList.Count;
    
    public int NumEdges
    {
        get
        {
            int count = _adjacencyList.Sum(node => node.Value.Count);
            return count / 2; // divide by 2 since edges are stored twice (once for each direction)
        }
    }
    
    public List<T> Vertices => _adjacencyList.Keys.ToList();
    ...
    {
        List<(T, T)> edges = new();
        foreach ((T source, List<T> value) in _adjacencyList)
        {
            foreach (T destination in value)
            {
                (T source, T destination) edge = (source, destination);
                if (!edges.Contains(edge) && !edges.Contains((destination, source)))
                {
                    edges.Add(edge);
                }
            }
    ...
    {
        return !_adjacencyList.ContainsKey(vertex) ? new List<T>() : new List<T>(_adjacencyList[vertex]);
    }
    ...
    /// MORE memory than DFS to store the queue but LESS memory to call the stack.
    /// </summary>
